...
for ((i=1; i<=$N; i++ )) do echo " -------- Kill mongod on ngd$i -------"; ssh -t 10.1.$i.2 "sudo killall mongod"; done

sleep 2

# Delete databases
for ((i=1; i<=$N; i++ )) do echo " -------- Delete databases on ngd$i -------"; ssh -t 10.1.$i.2 "sudo rm -r /mnt/mongo/localdata/*;sudo rm /home/ngd/mongo.log"; done
